...
题目与题解块用于算法、数学、教程类文章中的例题展示。它们与 `note`、`tip` 等提示块语法相似，但语义独立：

- `problem`：展示题目、例题、习题等内容。
- `solution`：展示题解，默认折叠，点击标题后展开。

#### 基本语法

```markdown
:::problem[例1]
给定集合 $A=\{1,2\}$，$B=\{2,3\}$。从每个集合中各选一个数，有多少种选法能使得两数之和为 $n$？
:::

:::solution[题解]
答案是 $[x^n](x+x^2)(x^2+x^3)$。
:::
```
...
$$
(x+x^2)(x^2+x^3)=x^3+2x^4+x^5
$$

因此当 $n=3,4,5$ 时，答案分别为 $1,2,1$，其他情况下答案为 $0$。
:::
